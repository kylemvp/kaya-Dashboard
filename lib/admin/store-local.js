...
const KEYS = {
  ...
  //
  // v2: a shipped build wrote the v1 marker while collections were still
  // empty (it re-read the marker after seeding the first collection, so that
  // write vouched for every stale one behind it). Anyone carrying that state
  // has "this is deliberate" recorded against an empty catalogue, and no
  // amount of reload fixes it. Bumping the key retires those markers so the
  // empty collections are seeded once more.
  seeded: 'kaya_admin_seeded_v2',
}
...
/** Save one country's overrides for one section, mirroring the Supabase call. */
...
export function resetDemo() {
  if (!isBrowser()) return
  // Retired marker from a previous build — clear it so it can't linger.
  try { window.localStorage.removeItem('kaya_admin_seeded_v1') } catch { /* ignore */ }
